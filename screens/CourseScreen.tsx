...
export default function CourseWeb() {
  ...
  const setFormField = <K extends keyof CourseFormData>(
    key: K,
          ...
          if (status === window.kakao.maps.services.Status.OK && data) {
            setModalSearchResults(
              data.map((r: any) => ({
                ...
                geometry: {
                  location: { lat: parseFloat(r.y), lng: parseFloat(r.x) },
                },
              })),
            );
  ...
  const updateFixedScheduleItem = (
    id: string,
    field: keyof FixedScheduleItem,
    value: string,
  ) => {
    setFormData((prev) => ({
      ...prev,
  ...
  const validateFixedScheduleItem = (
    ...
    if (!isTimeEndAfterStart(item.startTime, item.endTime))
  ...
  useEffect(() => {
    ...
    if (modalSearchQuery.trim()) {
      modalSearchTimeoutRef.current = setTimeout(
        () => modalSearchPlaces(modalSearchQuery),
        500,
      );
    } else {
      setModalSearchResults([]);
  ...
  const handleGenerate = async () => {
    ...
    if (!start || !end) {
      Toast.show({
        type: "error",
        text1: "알림",
        text2: "올바른 날짜 형식(YYYY-MM-DD)으로 입력해주세요.",
        position: "top",
        visibilityTime: 3000,
      });
      return;
    ...
    if (end < start) {
      Toast.show({
        type: "error",
        text1: "알림",
        text2: "종료 일자는 시작 일자보다 빠를 수 없습니다.",
        position: "top",
        visibilityTime: 3000,
      });
      return;
    ...
    if (
      ...
      formData.firstDayStartTime === formData.lastDayEndTime
    ) {
      Toast.show({
        type: "error",
        text1: "알림",
        text2: "시작 시간과 종료 시간이 같으면 코스를 생성할 수 없습니다.",
        position: "top",
        visibilityTime: 3000,
      });
      return;
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <Pressable
            style={[
              styles.fixedScheduleRow,
              isGenerating && styles.inputDisabled,
            ]}
            onPress={() =>
              !isGenerating && setIsFixedSchedule(!isFixedSchedule)
            }
            disabled={isGenerating}
          >
            <View
              style={[
                styles.checkboxBox,
                isFixedSchedule && styles.checkboxBoxChecked,
              ]}
            >
              {isFixedSchedule && (
          ...
          {isFixedSchedule && (
            <View style={styles.fixedScheduleContent}>
              <Pressable
                style={[
                  styles.addFixedButton,
                  isGenerating && styles.inputDisabled,
                ]}
                onPress={() => !isGenerating && addFixedScheduleItem()}
                disabled={isGenerating}
              >
                  ...
                  {formData.fixedSchedules.map((item) => {
                    ...
                    return (
                      <View
                        key={item.id}
                        style={[
                          styles.fixedScheduleCard,
                          styles.fixedScheduleCardSlide,
                        ]}
                      >
                        <View style={styles.fixedScheduleCardHeader}>
                          <Text style={styles.fixedScheduleCardTitle}>
                            {item.title || "고정 일정"}
                          </Text>
                          <Pressable
                            style={styles.fixedScheduleRemove}
                            onPress={() =>
                              !isGenerating && removeFixedScheduleItem(item.id)
                            }
                            disabled={isGenerating}
                          >
                            <Ionicons
                        ...
                        {item.placeName || item.address ? (
                          <View style={styles.fixedSchedulePlaceBlock}>
                            <Ionicons
                              ...
                              size={16}
                              color="#6366f1"
                              style={styles.fixedSchedulePlaceIcon}
                            />
                            <View style={styles.fixedSchedulePlaceText}>
                              {item.placeName ? (
                                <Text
                                  style={styles.fixedSchedulePlaceName}
                                  numberOfLines={1}
                                >
                                  {item.placeName}
                                </Text>
                              ...
                              {item.address && String(item.address).trim() ? (
                                <Text
                                  style={styles.fixedSchedulePlaceAddr}
                                  numberOfLines={1}
                                >
                                  {item.address}
                                </Text>
                        ...
                        <View style={styles.inputGroup}>
                          ...
                          <WebDateInput
                            value={item.date}
                            onChange={(v) =>
                              updateFixedScheduleItem(item.id, "date", v)
                            }
                            disabled={isGenerating}
                        ...
                        <View style={styles.inputGroup}>
                          ...
                          <TextInput
                            ...
                            value={item.title}
                            onChangeText={(v) =>
                              updateFixedScheduleItem(item.id, "title", v)
                            }
                        ...
                        <View style={[styles.row, styles.rowTimeInputs]}>
                          <View style={styles.inputGroupFlex}>
                            ...
                            <WebTimeInput
                              ...
                              onChange={(v) =>
                                updateFixedScheduleItem(item.id, "startTime", v)
                              }
                              disabled={isGenerating}
                              placeholder="14:00"
                            />
                          ...
                          <View style={styles.inputGroupFlex}>
                            ...
                            <WebTimeInput
                              value={item.endTime}
                              onChange={(v) =>
                                updateFixedScheduleItem(item.id, "endTime", v)
                              }
                              disabled={isGenerating}
                              hasError={!!err}
                              placeholder="16:00"
                ...
                formData.fixedSchedules.map((item) => {
                  ...
                  return (
                    <View
                      ...
                      style={[
                        styles.fixedScheduleCard,
                        styles.fixedScheduleCardSingle,
                      ]}
                    >
                      <View style={styles.fixedScheduleCardHeader}>
                        <Text style={styles.fixedScheduleCardTitle}>
            ...
            <View style={styles.pillContainer}>
              {purposes.map((purpose) => (
                <Pressable
                  ...
                  style={[
                    styles.pill,
                    selectedPurposes.includes(purpose) && styles.pillActive,
                    isGenerating && styles.inputDisabled,
                  ]}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.inputGroup}>
            ...
            <View style={styles.buttonRow}>
              {[
                { key: "walkAndPublic", label: "도보 + 대중교통" },
                { key: "car", label: "차량" },
              ].map((option) => (
                <Pressable
                  key={option.key}
        ...
        <View style={styles.generateSection}>
          ...
          >
            ...
            <View style={styles.generateList}>
              <Text style={styles.generateItem}>
                1 조건에 맞는 추천 스팟 리스트 (카테고리별)
              </Text>
              <Text style={styles.generateItem}>
                2 시작점 기준 최적 동선 코스 A안 · 교통 반영
              </Text>
              <Text style={styles.generateItem}>
                3 지도 기반 일정표 + 타임라인 한 화면 제공
              </Text>
            </View>
            <Pressable
              style={[
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.districtList}>
              <Pressable
                style={[
                  styles.districtItem,
                  selectedDistrict === "" && styles.districtItemActive,
                ]}
                onPress={() => {
                  setSelectedDistrict("");
      ...
      >
        ...
        >
          ...
          <ScrollView
            style={styles.addFixedModalScroll}
            contentContainerStyle={styles.addFixedModalScrollContent}
          ...
          >
            ...
            <View style={styles.addFixedModalFormSection}>
              ...
              <View style={styles.addFixedModalActions}>
                <Pressable
                  style={styles.addFixedCancelButton}
                  onPress={closeAddFixedModal}
                >
